      ...
      if (fotos && fotos.length > 0) {
        ...
        for (let i = 0; i < fotos.length; i++) {
          ...
          try {
            ...
            const imgData = await cargarImagenComoBase64(foto.url_foto)
            ...
            if (imgData) {
              ...
              // Calcular dimensiones óptimas (máximo ancho: 170mm, máximo alto: 100mm)
              const maxWidth = 170
              const maxHeight = 100
              ...
              if (yPos + height + 15 > 270) {
                doc.addPage()
                yPos = 20
              }
              
              // Agregar etiqueta de la foto
              doc.setFont('helvetica', 'normal')
              doc.setFontSize(9)
              doc.text(`Foto ${i + 1}${foto.descripcion ? ': ' + foto.descripcion : ''}`, 20, yPos)
              yPos += 5
            ...
            // Continuar con la siguiente foto si hay un error
          }
        }
      }
