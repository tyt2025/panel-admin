...
export default function DetalleServicioTaller({ params }) {
  ...
  async function generarPDF() {
    ...
    try {
      ...
      doc.text('TINTAS Y TECNOLOGÍA', pageWidth / 2, 15, { align: 'center' })
      
      doc.setFontSize(12)
      doc.setFont('helvetica', 'normal')
      doc.text('Comprobante de Servicio Técnico', pageWidth / 2, 25, { align: 'center' })
      doc.text('Calle 14 # 5-44 local 1 Barrio Centro, Santa Marta', pageWidth / 2, 32, { align: 'center' })
      doc.text('WhatsApp: 3102605693 - 3122405144', pageWidth / 2, 37, { align: 'center' })
      ...
      doc.setFontSize(11)
      doc.setFont('helvetica', 'bold')
      doc.text('DATOS DEL CLIENTE', margin + 2, yPos + 5.5)
      ...
      doc.setFontSize(11)
      doc.setFont('helvetica', 'bold')
      doc.text('DATOS DEL EQUIPO', margin + 2, yPos + 5.5)
      ...
      // Agregar Serial si está en las observaciones o crear un campo genérico
      if (servicio.observaciones && servicio.observaciones.toLowerCase().includes('serial')) {
        yPos += 6
        doc.text(`Serial: Ver observaciones`, margin + 2, yPos)
      }
      ...
      doc.setFontSize(11)
      doc.setFont('helvetica', 'bold')
      doc.text('ACCESORIOS INCLUIDOS', margin + 2, yPos + 5.5)
      ...
      if (servicio.observaciones && servicio.observaciones.trim() !== '') {
        ...
        doc.setFontSize(11)
        doc.setFont('helvetica', 'bold')
        doc.text('OBSERVACIONES', margin + 2, yPos + 5.5)

        yPos += 12
        doc.setFontSize(10)
        doc.setFont('helvetica', 'normal')
        
        // Dividir las observaciones en líneas para que no se salgan del margen
        const maxWidth = pageWidth - 2 * margin - 4
        const observacionesLines = doc.splitTextToSize(servicio.observaciones, maxWidth)
        
        observacionesLines.forEach(line => {
          // Verificar si necesitamos más espacio antes del footer
          if (yPos > pageHeight - 80) {
            yPos = pageHeight - 80 // Limitar para no chocar con info importante
          }
      ...
      // Sección: INFORMACIÓN DE PAGO (NUEVO)
      if (servicio.valor_servicio > 0) {
        ...
        doc.setFontSize(11)
        doc.setFont('helvetica', 'bold')
        doc.text('INFORMACION DE PAGO', margin + 2, yPos + 5.5)
        ...
        if (totalPagar > 0) {
          ...
          doc.text(`SALDO PENDIENTE: ${formatearMoneda(totalPagar)}`, margin + 2, yPos + 2)
        } else {
          doc.setFillColor(220, 252, 231) // Verde claro
          doc.rect(margin, yPos - 5, pageWidth - 2 * margin, 10, 'F')
          doc.setTextColor(21, 128, 61)
          doc.text(`PAGADO COMPLETAMENTE`, margin + 2, yPos + 2)
        }
        doc.setTextColor(0, 0, 0)
        yPos += 12
      }
      ...
      doc.setFontSize(10)
      doc.setFont('helvetica', 'bold')
      doc.text('INFORMACIÓN IMPORTANTE', margin + 2, yPos + 6)
      ...
      yPos += 12
      doc.text('• Según la Ley 1480 de 2011, los equipos no reclamados después de 90 días', margin + 4, yPos)
      yPos += 5
      doc.text('  se consideran abandonados y pueden ser dispuestos por el establecimiento.', margin + 4, yPos)
      yPos += 7
      doc.setFont('helvetica', 'bold')
      doc.text(`• Días transcurridos: ${dias} | Días restantes: ${diasRestantes}`, margin + 4, yPos)
      doc.setFont('helvetica', 'normal')
      yPos += 7
      doc.text('• Conserve este comprobante para reclamar su equipo.', margin + 4, yPos)

      // Footer
      yPos = pageHeight - 20
      doc.setFontSize(8)
      doc.setTextColor(100, 100, 100)
      doc.text('Tintas y Tecnología | tintasytecnologia2018@gmail.com', pageWidth / 2, yPos, { align: 'center' })
      yPos += 4
      doc.text('www.tintasytecnologia.com | Horario: Lun-Sáb 8:00am-6:20pm', pageWidth / 2, yPos, { align: 'center' })
      yPos += 4
      doc.text(`Página 1 de ${servicio.fotos && servicio.fotos.length > 0 ? '2' : '1'} | Servicio #${servicio.id}`, pageWidth / 2, yPos, { align: 'center' })
