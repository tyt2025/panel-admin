...
export default function EditarCotizacion() {
  ...
  const loadData = async () => {
    try {
      ...
      console.log('📦 Productos disponibles:', productosOrdenados.slice(0, 5).map(p => ({id: p.id, name: p.product_name, sku: p.sku})))
  ...
  const productosFiltrados = (productos || []).filter(p => {
    ...
    const nombreNormalized = normalizeText(p.product_name || '')
