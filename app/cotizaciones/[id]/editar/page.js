...
export default function EditarCotizacion() {
  ...
  const loadData = async () => {
    try {
      ...
      const [cotRes, itemsRes, clientesRes, productosRes] = await Promise.all([
        supabase
          .from('cotizaciones')
          .select('*, clientes(*)')
          .eq('id', params.id)
      ...
      if (cotData) {
        ...
        // Cargar datos de domicilio si existen (en paralelo también)
        if (cotData.delivery_id) {
          setIncluirDomicilio(true)
          // Cargar sin await para no bloquear
          supabase
            .from('delivery_rates')
            .select('*')
  ...
  const guardarCambios = async () => {
    ...
    try {
      ...
      // Actualizar cotización (SIN campos de desglose IVA - agregar después de ejecutar SQL)
