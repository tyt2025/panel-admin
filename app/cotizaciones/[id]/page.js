...
export default function VerCotizacion() {
  ...
  const generarPDF = async () => {
    try {
      ...
      loadingMsg.innerHTML = '<div style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:rgba(0,0,0,0.9);color:white;padding:25px 40px;border-radius:12px;z-index:9999;font-size:20px;font-weight:bold;box-shadow:0 4px 20px rgba(0,0,0,0.5)">⚡ Generando PDF rápido...</div>'
      document.body.appendChild(loadingMsg)

      const doc = new jsPDF()
      
      // Configuración de colores
      const primaryColor = [240, 0, 0]
      const textColor = [51, 51, 51]
      
      // OPTIMIZACIÓN EXTREMA: Cargar imágenes con timeout MUY CORTO
      const loadImageUltraFast = async (url, timeout = 800) => {
        try {
      ...
      // Cargar SOLO imágenes esenciales en paralelo
      const [logoImg, domicilioImg] = await Promise.all([
        loadImageUltraFast('https://cxxifwpwarbrrodtzyqn.supabase.co/storage/v1/object/public/Logo/logo%20circulo%20(1).png'),
        cotizacion.delivery_id ? loadImageUltraFast('https://cxxifwpwarbrrodtzyqn.supabase.co/storage/v1/object/public/Logo/domicilio.png') : null
      ])
      ...
      // Items - CARGAR IMÁGENES UNA POR UNA RÁPIDO
      doc.setFont('helvetica', 'normal')
      for (let index = 0; index < items.length; index++) {
        ...
        // Cargar imagen del producto RÁPIDO (solo 600ms de espera)
        if (item.productos?.image_url_png) {
          const productImg = await loadImageUltraFast(item.productos.image_url_png, 600)
          if (productImg) {
            try {
              doc.addImage(productImg, 'PNG', 22, startY, 20, 20)
            } catch (e) {
      ...
      if (cotizacion.delivery_id && cotizacion.delivery_rates) {
        ...
        if (domicilioImg) {
          doc.addImage(domicilioImg, 'PNG', 22, startY, 20, 20)
        } else {
          doc.setFillColor(240, 240, 240)
          doc.rect(22, startY, 20, 20, 'F')
          doc.setFont('helvetica', 'bold')
      ...
      // Desglose IVA
      if (cotizacion.mostrar_desglose_iva) {
        doc.text('Base imponible:', 120, yPos)
        doc.text(`$${cotizacion.base_imponible?.toLocaleString('es-CO', {minimumFractionDigits: 0, maximumFractionDigits: 0})}`, 185, yPos, { align: 'right' })
        yPos += 6
        doc.text('IVA (19%):', 120, yPos)
        doc.text(`$${cotizacion.iva_monto?.toLocaleString('es-CO', {minimumFractionDigits: 0, maximumFractionDigits: 0})}`, 185, yPos, { align: 'right' })
        yPos += 6
        doc.setFont('helvetica', 'bold')
        doc.text('Subtotal (con IVA):', 120, yPos)
      ...
      if (cotizacion.delivery_id && cotizacion.delivery_rates) {
        ...
        doc.text('Envío a domicilio:', 120, yPos)
        doc.text(`+$${cotizacion.delivery_rates.price_cop?.toLocaleString('es-CO')}`, 185, yPos, { align: 'right' })
        doc.setTextColor(...textColor)
        yPos += 8
      }
      ...
      const msgElement = document.getElementById('pdf-loading')
    ...
    <>
      ...
      <div ref={printRef} style={{ position: 'absolute', left: '-9999px', width: '800px', backgroundColor: 'white', padding: '40px' }}>
        ...
        <div style={{ marginLeft: 'auto', width: '400px', backgroundColor: '#f9fafb', padding: '25px', borderRadius: '8px', marginBottom: '30px' }}>
          {cotizacion.mostrar_desglose_iva && (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', fontSize: '14px' }}>
                <span>Base imponible:</span>
                <span style={{ fontWeight: 'bold' }}>${cotizacion.base_imponible?.toLocaleString('es-CO', {minimumFractionDigits: 0, maximumFractionDigits: 0})}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', fontSize: '14px' }}>
                <span>IVA (19%):</span>
                <span style={{ fontWeight: 'bold' }}>${cotizacion.iva_monto?.toLocaleString('es-CO', {minimumFractionDigits: 0, maximumFractionDigits: 0})}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '18px', fontSize: '14px', fontWeight: 'bold' }}>
                <span>Subtotal (con IVA):</span>
                <span>${cotizacion.subtotal?.toLocaleString('es-CO')}</span>
              </div>
            </>
          )}
          {cotizacion.descuento > 0 && (
