...
export default function NuevaCotizacion() {
  ...
  const loadData = async (userData) => {
    try {
      ...
      console.log('📦 Primeros 3 productos:', productosOrdenados.slice(0, 3).map(p => ({id: p.id, name: p.product_name, sku: p.sku})))
      console.log('📦 Últimos 3 productos:', productosOrdenados.slice(-3).map(p => ({id: p.id, name: p.product_name, sku: p.sku})))
  ...
  const calcularTotales = () => {
    ...
    // Calcular retenciones (se aplican sobre la base antes de descuentos)
    ...
    try {
      ...
      const { data: cotizacion, error: cotError} = await supabase
        ...
        .insert([{
          ...
          estado: 'pendiente',
          // Retenciones
          retefuente: aplicarRetefuente ? retefuente : 0,
          reteiva: aplicarReteiva ? reteiva : 0,
          ica: aplicarIca ? ica : 0,
          reteica: aplicarReteica ? reteica : 0,
          monto_retefuente: totales.montoRetefuente,
          monto_reteiva: totales.montoReteiva,
          monto_ica: totales.montoIca,
          monto_reteica: totales.montoReteica
        }])
        .select()
        .single()
  ...
  const productosFiltrados = productos.filter(p => {
    ...
    const nombreNormalized = normalizeText(p.product_name || p.name || '')
